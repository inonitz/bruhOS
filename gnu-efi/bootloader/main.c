...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SystemTable)
{
    ...
    entry = loadElf64(kfhandle, &kpstart, &kvstart, &ksize, &kstackp, &kstacksz);
    
    get_verify_acpi_rsdp(&rsdp);                           // get the rsdt pointer, such that the kernel will be able to use acpi
    kinf.acpi.extended = rsdp->first.revision == 2;        // initialize the kernel header with the RSDT info we found.
    kinf.acpi.address  = (rsdp->xsdtAddress       *  kinf.acpi.extended) // ^^^^
                       + (rsdp->first.rsdtAddress * !kinf.acpi.extended); 

    initialize_screenbuffer((vec2us){0, 0}, &kinf.screen); // initialize the buffer needed to print to screen.
    kinf.stack.rbpAddress = kstackp;                                 // set the new rbp pointer, so that the kernel gets a new stack.
    kinf.stack.size       = kstacksz;                                // set the size of the rbp stack.
    ...
    enablePagingJumpToElf64Ver3(&kinf, entry, kpstart, kvstart);
