...
{
    KERNEL_ERROR_IF(acpi_config->extended == BOOLEAN_FALSE, KERNEL_NOT_IMPLEMENTED, BOOLEAN_TRUE, "ACPI 1.0 and below isn't supported. Exiting...\n");
    ...
    bool_t               lgcy_irqs   = BOOLEAN_FALSE; // if this is true, disable the PIC.
    const char_t*        hdrname  = "    ";
    const char_t*        madt_sig = "APIC";
    const char_t*        hpet_sig = "HPET";
    ...
    for(uint8_t i = 0; i < entries; ++i)
    {
        ...
        memcpy((void*)hdrname, hdr->signature, 4);
    ...
    KERNEL_ERROR_IF(idxreg0 == UINT32_MAX, KERNEL_MISSING_DATA, BOOLEAN_TRUE, "    ACPI MADT       Table Not Found.");
    KERNEL_ERROR_IF(idxreg1 == UINT32_MAX, KERNEL_MISSING_DATA, BOOLEAN_TRUE, "    ACPI HPET Timer Table Not Found.");
    madt = VIRT_TYPE(xsdt_table->table[idxreg0], volatile MADT_t*);
    hpet = VIRT_TYPE(xsdt_table->table[idxreg1], volatile HPET_t*);
    KERNEL_ERROR_IF(tableChecksum(&((MADT_t*)madt)->hdr) != 1, KERNEL_BAD_DATA, BOOLEAN_TRUE, "    ACPI MADT Table is invalid.");
    KERNEL_ERROR_IF(tableChecksum(&((HPET_t*)hpet)->hdr) != 1, KERNEL_BAD_DATA, BOOLEAN_TRUE, "    ACPI HPET Table is invalid.");
