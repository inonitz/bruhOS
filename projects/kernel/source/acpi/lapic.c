...
static LOGICAL uintptr_t lapic_base;
...
__force_inline uint32_t lapic_read_reg(uint16_t offset)
{
    return *(uint32_t volatile*)((uint8_t*)lapic_base + offset); 
}


__force_inline void lapic_write_reg(uint16_t offset, uint32_t data)
{
...
{
    uint32_t icr1 = (lapic_read_reg(LAPIC_INTCOMMND1) & 0x00ffffff) | ((lapic_id & 0xFF) << 24);
    uint32_t icr0 = ( (lapic_read_reg(LAPIC_INTCOMMND0) & 0xfff00000) | (trampoline >> LOG2_PAGE_SIZE) ) | ( (ICR_DELIVMODE_SIPI << 8) | (ICR_DESTMODE_TYPE_PHYSICAL << 11) );
    debug_printk("icr1 %X\nicr0 %X\n\n", icr1, icr0);
...
__force_inline void lapic_send_eoi()
{
    lapic_write_reg(LAPIC_EOI, 0);
}


__force_inline uint32_t lapic_get_id()
{
...
__force_inline bool_t lapic_pending()
{
