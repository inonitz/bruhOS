 ...
 * or double) rotation if necessary.  The return value will be %BOOLEAN_TRUE if
 * the full AVL tree is now adequately balanced, or %BOOLEAN_FALSE if the subtree
 * rooted at @parent is now adequately balanced but has increased in
 * height by 1, so the caller should continue up the tree.
 *
 * Note that if %BOOLEAN_FALSE is returned, no rotation will have been done.
 ...
 *
 * @left_deleted_ret
 *	If the return value is not NULL, this will be set to %BOOLEAN_TRUE if the
 *	left subtree of the returned node has decreased in height by 1,
 *	or %BOOLEAN_FALSE if the right subtree of the returned node has decreased
 *	in height by 1.
 *
 * This function will adjust @parent's balance factor, then do a (single
			     ...
			     bool_t *left_deleted_ret)
{
	...
	if (!Y->left) {
		 ...
		 */
		ret = Y;
		*left_deleted_ret = BOOLEAN_FALSE;
	} else {
...
{
	struct avl_tree_node *parent;
	bool_t left_deleted = BOOLEAN_FALSE;
	...
	if (node->left && node->right) {
		 ...
		 * @left_deleted has been set to %BOOLEAN_TRUE if @node's
		 * in-order successor was the left child of @parent,
		 * otherwise %BOOLEAN_FALSE.  */
	} else {
		struct avl_tree_node *child;
		...
		if (parent) {
			if (node == parent->left) {
				parent->left = child;
				left_deleted = BOOLEAN_TRUE;
			} else {
				parent->right = child;
				left_deleted = BOOLEAN_FALSE;
			}
			if (child)
