...
uint64_t write_array_to_framebuffer(
	IN __attribute__((unused)) framebuffer_t* fb,
	IN __attribute__((unused)) vec2us         positions[],
	IN __attribute__((unused)) uint32_t       colors[],
	IN __attribute__((unused)) vec2us         pixel_offset
)
{
	...
	while(begin < end)
	{
		...
		for(int8_t y = 0; y < style->GlyphHeight; ++y) // for each line in bitmap
		{
			for(int8_t x = 0; x < style->GlyphBytesWidth; ++x) // draw every line
			{
				for(uint8_t i = 0; i < 8u; ++i)
				{
					masks[i]  = boolean(*chline & (0x80u >> i));
					masks[i] *= color;
					masks[i] += !masks[i] * backColor;
				}
				memcpy(pixel_address, masks, sizeof(masks));
