...
static void lock(spinlock_t* sl)
{
    for(;;)
    {
        if(!atomic_exchange_ret_explicit_u8(&sl->flag, BOOLEAN_TRUE, memory_model_acquire)) {
            return;
        }
...
/* 
    ...
    * BOOLEAN_TRUE  - Lock Acquired successfully.
    * FALSE - failed to acquire the lock, someone is using it.
*/
static __force_inline bool_t try_lock(spinlock_t* sl)
{
    return !atomic_load_u8(&sl->flag) && !atomic_exchange_ret_u8(&sl->flag, BOOLEAN_TRUE);
}
