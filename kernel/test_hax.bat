...
qemu-system-x86_64 -accel hax -drive format=raw,file=%BUILDDIR%\%OSNAME%.img -m 2G -drive if=pflash,format=raw,unit=0,file=%OVMFDIR%\OVMF_CODE-pure-efi.fd,readonly=on -drive if=pflash,format=raw,unit=1,file=%OVMFDIR%\OVMF_VARS-pure-efi.fd -net none
pause

// -cpu max,+tsc-deadline
// -accel hax doesn't work for some reason :(
