...
static volatile bool_t  polled = FALSE;
...
void HPET_init(volatile HPET_t* table)
{
    ...
    toVirtual((void*)table->registers.address, 1);
    ...
    cfg &= ~CONFIG_REG_ENABLE_TIMER;                      // enable_timer   = FALSE
    cfg &= ~CONFIG_REG_REPLACE_LEGACY;                    // legacy_replace = FALSE
    ...
    installISA(ioapic_irq0, 0, PRI_LVL0, (void*)(uint64_t)hpet_timer_handler);  // send to cpu0 (BSP) timer interrupts.
...
bruh = poll_timer_count();
while(poll_timer_count() < 0x51dac207a000) {}
hang();
for(uint64_t i = 0; i < 200; ++i)
{
    for(uint64_t j = 0; j < 10000; ++j) { __asm__ volatile("pause"); }
    printk("%X ", poll_timer_count());
    if((1+i) % 10 == 0) {
        putln();
    }
}
