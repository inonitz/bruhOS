...
void pfa_init(efi_memory_map* map)
{
    ...
    totalAllocators   = ((MemoryMapPageCount * 4096ull) >> (BUDDY_TREE_DEPTH + 12ull));           
    ...
    KERNEL_ERROR_IF(biggestFree == nullptr, BUFFER_TOO_SMALL, TRUE, "PageFrameAllocator Couldn't initialize - couldn't find enough space for the memory allocators.");
    ...
    for(uint32_t i = 0; i < totalAllocators; ++i) {
        ...
        pageManager.zoneAllocators[i].using   = FALSE;
    }
    ...
    pageManager.lowZoneAllocator.using   = FALSE;
    ...
    memBoundaries[0] = 0;
    for(uint16_t i = 1; i < totalAllocators + 1; ++i) {
...
void* pfa_alloc_pages(IN uint32_t count, uint8_t type)
{
    ...
    void*          out       = nullptr;
    bool_t         satisfied = FALSE;
    request_type_t req       = (request_type_t){ .u8 = type };
    if(unlikely(req.mem_location == DMA_LOW))
    {
        // if someone is using this low-zone allocator, we'll wait.
        while(pageManager.lowZoneAllocator.using) { __asm__ volatile("pause" : : : "memory"); }

        atomic_exchange_ret_u8(&pageManager.lowZoneAllocator.using, TRUE);     // atomic lock. 
        out = lowmem_alloc_pages(pageManager.lowZoneAllocator.manager, count); // allocate pages.
        satisfied = boolean(out);                                              // set satisfied.
        atomic_clear_u8(&pageManager.lowZoneAllocator.using);                  // atomic clear
        return out;
    }
    ...
    for(uint16_t i = 0; !satisfied && i < pageManager.zoneCount; ++i)
    {
        ...
        while(pageManager.zoneAllocators[i].using || pageManager.zoneAllocators[i].maxMemReq < count) {
            continue;
        }

        // atomic lock. 
        atomic_exchange_ret_u8(&pageManager.zoneAllocators[i].using, TRUE);
        ...
        // atomic clear.
        atomic_clear_u8(&pageManager.zoneAllocators[i].using);
    }
    return out;
}
...
bool_t pfa_free(IN void* ptr, uint32_t blockSize)
{
    ...
    if((uint64_t)ptr < 0x400000) { // this is a low mem request.
        ...
        while(pageManager.lowZoneAllocator.using) {    // if someone is already using this allocator, we wait for them.
            __asm__ volatile("pause" : : : "memory"); 
        }


        // atomic lock
        atomic_exchange_ret_u8(&pageManager.lowZoneAllocator.using, TRUE);
        
        freed = lowmem_free_pages(pageManager.lowZoneAllocator.manager, ptr, lvl);
        if(unlikely(freed != SUCCESS)) {
            KERNEL_ERROR(INVALID_INPUT, FALSE, "Failed to Free Pages - Invalid Input Passed to Function.")
        ...
        // atomic clear
        atomic_clear_u8(&pageManager.lowZoneAllocator.using);
    }
    else {
        ...
        // if someone is already using allocator[alloc_idx], we wait for them.
        while(pageManager.zoneAllocators[alloc_idx].using) { 
            __asm__ volatile("pause" : : : "memory");
        }


        // atomic lock.
        atomic_exchange_ret_u8(&pageManager.zoneAllocators[alloc_idx].using, TRUE);  
        ...
        // atomic clear.
        atomic_clear_u8(&pageManager.zoneAllocators[alloc_idx].using);
    }
...
// )
// {
//     const size_t           dscs              = map->map_size / map->entry_size;
//     efi_mem_descriptor*    dsc               = 0;
//     uint64_t               size              = 0;
