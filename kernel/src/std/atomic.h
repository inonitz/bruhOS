...
typedef int8_t   align(sizeof(int8_t  )) atomic8_t  ; 
typedef int16_t  align(sizeof(int16_t )) atomic16_t ; 
typedef int32_t  align(sizeof(int32_t )) atomic32_t ; 
typedef int64_t  align(sizeof(int64_t )) atomic64_t ; 
typedef uint8_t  align(sizeof(uint8_t )) atomic8u_t ; 
typedef uint16_t align(sizeof(uint16_t)) atomic16u_t; 
typedef uint32_t align(sizeof(uint32_t)) atomic32u_t; 
typedef uint64_t align(sizeof(uint64_t)) atomic64u_t; 
...
#define ATOMIC_SET_MACRO(ValueType, identifier_str) \
    static inline __always_inline void atomic_set_explicit_##identifier_str(ValueType* dest, ValueType* src, int32_t memOrder) \
    { \
        __atomic_load(src, dest, memOrder); \
    } \
\
\
    static inline __always_inline void atomic_set_##identifier_str(ValueType* dest, ValueType* src) \
    { \
...
#define ATOMIC_LOAD_MACRO(ValueType, identifier_str) \
    static inline __always_inline ValueType atomic_load_explicit_##identifier_str(ValueType* dest, int32_t memOrder) \
    { \
        return __atomic_load_n(dest, memOrder); \
    } \
\
\
    static inline __always_inline ValueType atomic_load_##identifier_str(ValueType* dest) \
    { \
...
#define ATOMIC_STORE_MACRO(ValueType, identifier_str) \
    static inline __always_inline void atomic_store_explicit_##identifier_str(ValueType* dest, ValueType* src, int32_t memOrder) \
    { \
...
\
\
    static inline __always_inline void atomic_store_##identifier_str(ValueType* dest, ValueType* src) \
    { \
...
#define ATOMIC_EXCHANGE_MACRO(ValueType, identifier_str) \
    static inline __always_inline void atomic_exchange_explicit_##identifier_str(ValueType* dest, ValueType* src, ValueType* destPrevVal, int32_t memOrder) \
    { \
...
\
\
    static inline __always_inline void atomic_exchange_##identifier_str(ValueType* dest, ValueType* src, ValueType* destPrevVal) \
    { \
...
\
\
    static inline __always_inline ValueType atomic_exchange_ret_explicit_##identifier_str(ValueType* dest, ValueType src, int32_t memOrder) \
    { \
...
\
\
    static inline __always_inline ValueType atomic_exchange_ret_##identifier_str(ValueType* dest, ValueType src) \
    { \
        /* destPrevVal is returned instead of being explicitly set with a pointer. */ \
...
#define ATOMIC_CMP_XCHG_MACRO(ValueType, identifier_str) \
    static inline __always_inline bool_t atomic_cmpxchg_explicit_##identifier_str(     \
        ValueType* cmpValue,             \
        ValueType* expected,             \
        ValueType  setTo,                \
        ...
        */ \
        return __atomic_compare_exchange_n(cmpValue, expected, setTo, FALSE, memOrderSuccess, memOrderFail); \
    } \
\
\
    static inline __always_inline bool_t atomic_cmpxchg_##identifier_str(   \
        ValueType* cmpValue, \
        ValueType* expected, \
        ValueType  setTo     \
    ) { \
        /*  if cmpValue == expected: 
                ...
                set expected to cmpValue 
        */ \
        return __atomic_compare_exchange_n(cmpValue, expected, setTo, FALSE, MEMORY_MODEL, MEMORY_MODEL); \
    } \
\
\
    static inline __always_inline bool_t atomic_cmpxchg_ptr_explicit_##identifier_str( \
        ValueType* cmpValue,            \
        ValueType* expected,            \
        ValueType* setTo,               \
        int32_t    memOrderSuccess,     \
        ...
        return __atomic_compare_exchange(cmpValue, expected, setTo, FALSE, memOrderSuccess, memOrderFail); \
    } \
\
\
    static inline __always_inline bool_t atomic_cmpxchg_ptr_##identifier_str( \
        ValueType* cmpValue, \
        ValueType* expected, \
        ValueType* setTo     \
    ) { \
        /* The same except that setTo isn't a variable, but a pointer to a variable that may set cmpValue */ \
        return __atomic_compare_exchange(cmpValue, expected, setTo, FALSE, MEMORY_MODEL, MEMORY_MODEL); \
    } \
...
#define ATOMIC_OPERATION_THEN_FETCH(OPERATION, ValueType, identifier_str) \
    /* 
        Perform OPERATION on dest.
        return the value in dest. 
        this is analogous to return_variable = ++counter
    */ \
\
\
    static inline __always_inline ValueType atomic_##OPERATION##_fetch_explicit_##identifier_str(ValueType* dest, ValueType src, int32_t memOrder) \
    { \
        return __atomic_##OPERATION##_fetch(dest, src, memOrder); \
    } \
\
\
    static inline __always_inline ValueType atomic_##OPERATION##_fetch_##identifier_str(ValueType* dest, ValueType src) \
    { \
...
#define ATOMIC_FETCH_THEN_OPERATION(OPERATION, ValueType, identifier_str) \
    /* 
        store ptr in var tmp.
        Perform OPERATION on dest.
        return the value in tmp. 
        this is analogous to return_variable = counter++
    */ \
\
\
    static inline __always_inline ValueType atomic_fetch_##OPERATION##_explicit_##identifier_str(ValueType* dest, ValueType src, int32_t memOrder) \
    { \
        return __atomic_fetch_##OPERATION(dest, src, memOrder); \
    } \
\
\
    static inline __always_inline ValueType atomic_fetch_##OPERATION##_##identifier_str(ValueType* dest, ValueType src) \
    { \
...
#define ATOMIC_TEST_SET_MACRO(ValueType, identifier_str) \
    static inline __always_inline bool_t atomic_test_and_set_explicit_##identifier_str(ValueType* ptr, int32_t memOrder) \
    { \
        /* 
            returns TRUE if ptr was set to some predefined constant (non-zero), during the atomic operation. 
            if ptr was set already, this will return FALSE 
        */ \
        return __atomic_test_and_set(ptr, memOrder); \
    } \
\
\
    static inline __always_inline bool_t atomic_test_and_set_##identifier_str(ValueType* ptr) \
    { \
        /* 
            returns TRUE if ptr was set to some predefined constant (non-zero), during the atomic operation. 
            if ptr was set already, this will return FALSE 
...
#define ATOMIC_CLEAR_MACRO(ValueType, identifier_str) \
    static inline __always_inline void atomic_clear_explicit_##identifier_str(ValueType* ptr, int32_t memOrder) \
    { \
        /* sets the variable to 0 atomically */ \
        __atomic_clear(ptr, memOrder); \
        return; \
    } \
    static inline __always_inline void atomic_clear_##identifier_str(ValueType* ptr) \
    { \
        /* sets the variable to 0 atomically */ \
...
#define ATOMIC_THREAD_FENCE() \
    static inline __always_inline void atomic_thread_fence_explicit(int32_t memOrder) \
    { \
...
\
\
    static inline __always_inline void atomic_thread_fence() \
    { \
...
#define ATOMIC_SIGNAL_FENCE() \
    static inline __always_inline void atomic_signal_fence_explicit(int32_t memOrder) \
    { \
...
\
\
    static inline __always_inline void atomic_signal_fence() \
    { \
...
#define MEMORY_PAUSE() \
    static inline __always_inline void pause() \
    { \
