...
void print_internal(char_t* string, va_list arglist)
{
	uint64_t argval;
	uint32_t start    = 0; 
	uint32_t end 	  = 0;
	bool_t   dontRes  = FALSE;
	char_t   endl     = '\0';
		...
		switch(*c)
		{
			...
			case '%':
				...
				switch(*++c)
				{
					...
					intToStr = formatx(argval,  ITSbufSize, FALSE, intToStrBuffer);
					break;

					case 'X':
					argval   = va_arg(arglist, uint64_t);
					intToStr = formatx(argval,  ITSbufSize, TRUE, intToStrBuffer);
					break;
					...
					dontRes = TRUE;		    // we didn't change the buffer, so we don't reset it.
...
void 	 printk_align_format(uint16_t padding_amount) { format_align_pad = padding_amount; }
uint16_t printk_get_align   (						) { return format_align_pad; 		   }
