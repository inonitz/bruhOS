...
void smp_main(trampoline_data* data)
{
...
bool_t wakeup_processor(uint8_t lapic_id, uint64_t __beginning, trampoline_data* trmpln)
{
...
{
    uint64_t         tbegin, tsize, bsp, ap, alloc_low;
    ...
    *protGdtBase += (uint32_t)alloc_low;                                  // update the base address, we can't do this in the trampoline.
    ...
    init->ready     = FALSE;
    init->lock.flag = FALSE;
    init->tssOff    = GDT_TS0;
    init->pml4      = getCR3();
    init->rbp64     = (uint64_t)khdr->memcfg.percpuRBP + PERCPU_STACK_SIZE; // top of stack.
    init->gdt64     = getKernelGDTP();
    init->idt64     = getKernelIDTR();
    init->entry     = (uint64_t)smp_main;

    debug_print("TRAMPOLINE CONFIG:\n    Address:         %x\n    Ready:           %u\n    TSS Offset:      %x\n    AP Core PML4:    %x\n    AP Core Stack:   %X\n    AP Core GDT(64): %X\n    AP Core IDT(64): %X\n    AP Core Main:    %X\n",
        alloc_low,
        init->ready,
        init->tssOff,
        init->pml4,
    ...
    for(; hdr < end; hdr = (int_cnt_header*)((uint64_t)hdr + hdr->length)) 
    {
        ...
        if(wakeup_processor(ap, alloc_low, init)) {
            ...
            system_status(SUCCESS, nullptr);
            putln();
        }
        

        done          = FALSE;
        init->ready   = FALSE;             // reset ready status
        init->tssOff += 0x08;              // goto next tss Offset
        init->rbp64  += PERCPU_STACK_SIZE; // goto next Stack Pointer [RBP].
